...
ruta_train = 'english-novels/'
lst_arch = os.listdir(ruta_train)
lst_arch = [x for x in lst_arch if x[:2]=='EN']
dic = []
ruta_salida = 'pairs.json'
for i in range(len(lst_arch)):
    f1 = open ('english-novels/%s/known01.txt'%lst_arch[i],'r')
    f2 = open ('english-novels/%s/unknown.txt'%lst_arch[i],'r')
    text1 = f1.read()
    text2 = f2.read()
...
with jsonlines.open(ruta_train+'pairs.jsonl', mode='w') as writer:
    for i in range(len(lst_arch)):
        writer.write(d[i])