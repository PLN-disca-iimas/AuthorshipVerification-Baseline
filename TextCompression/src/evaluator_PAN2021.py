...
"""
# Evaluation script for the Cross-Domain Authorship Verification task @PAN2021.
## Measures
...
>> python evaluator_PAN2021 -i COLLECTION -a ANSWERS -o OUTPUT
where
...
Example:
>> python evaluator_PAN2021.py -i "datasets/test_truth/truth.jsonl" \
        -a "out/answers.jsonl" \
        -o "pan21-evaluation"
...
def load_file(fn):
    ...
    for line in open(fn):
        ...
        try:
            problems[d['id']] = d['value']  # dictionary {id:value}
        except KeyError:
            problems[d['id']] = d['same']  # dictionary {id:value}
...
def evaluate_all(true_y, pred_y):
    """
    Convenience function: calculates all PAN21 evaluation measures
...
def main():
    ...
    parser.add_argument('-i', type=str, help='Path to the jsonl-file with ground truth scores')
    parser.add_argument('-a', type=str, help='Path to the jsonl-file with the answers (system prediction)')
    ...
    ans = load_file(f"{args.a}/answers.jsonl")

    print('->', len(gt), 'problems in ground truth')
    print('->', len(ans), 'solutions explicitly proposed')

    # default missing problems to 0.5
    for prob_id in sorted(gt.keys()):
        if prob_id not in ans.keys():
            print(f'El problema {prob_id} no fue resuelto')
            ans[prob_id] = 0.5

    # sanity check:
    assert len(gt) == len(ans)
    assert set(gt.keys()).union(set(ans.keys())) == set(gt.keys())

    # align the scores:
    scores = [(gt[k], ans[k]) for k in sorted(gt.keys())]
    gt, ans = zip(*scores)
